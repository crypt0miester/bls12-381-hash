...
use crate::g1_consts::{
    INV, ISO11_XDEN, ISO11_XNUM, ISO11_YDEN, ISO11_YNUM, MODULUS, R, R2, SSWU_ELLP_A, SSWU_ELLP_B,
    SSWU_XI,
};
...
fn map_to_curve_sswu(u: &Fp, c_neg_b_over_a: &Fp, exps: &Exps) -> Result<PointPrime, ProgramError> {
    ...
    let usq = mont_mul(&um, &um);
    let xi_usq = mont_mul(&SSWU_XI, &usq);
    let tv2 = add_mod(&mont_mul(&xi_usq, &xi_usq), &xi_usq);
...
fn add_prime(p: &PointPrime, q: &PointPrime, exps: &Exps) -> Result<PointPrime, ProgramError> {
    ...
    let x3 = sub_mod(&sub_mod(&mont_mul(&lambda, &lambda), &p.x), &q.x);
...
fn clear_cofactor(p: &[u8; 96]) -> Result<[u8; 96], ProgramError> {
    let mut acc = *p;
    for bit in (0..63).rev() {
        acc = g1_add(&acc, &acc)?;
        if (H_EFF >> bit) & 1 == 1 {
...
fn validate(p: &[u8; 96]) -> Result<(), ProgramError> {
    let mut out = 0u8;
...
fn point_bytes(x: &[u8; 48], y: &[u8; 48]) -> [u8; 96] {
    let mut out = [0u8; 96];
...
fn mac32(acc: u64, a: u64, b: u64, carry: u64) -> (u64, u64) {
    // operands hold 32-bit values, so this cannot overflow u64
    let t = acc + a * b + carry;
    (t & 0xffff_ffff, t >> 32)
}
...
fn hash_to_field_folded(msg: &[u8]) -> [FieldElem; 2] {
    ...
    for (i, elem) in out.iter_mut().enumerate() {
        ...
        let hi_m = to_mont(&be_to_limbs(&hi));
        let lo_m = to_mont(&be_to_limbs(&lo));
        let u_m = add_mod(&mont_mul(&hi_m, &c256), &lo_m);
        elem.canonical = from_mont(&u_m);
        elem.mont = u_m;
    }
    out
}
...
fn check_inverse(v_m: &Fp, witness: &Fp) -> Result<Fp, ProgramError> {
    let w_m = to_mont(witness);
    if mont_mul(v_m, &w_m) != R {
        return Err(ProgramError::InvalidInstructionData);
    }
    Ok(w_m)
}
...
fn map_to_curve_witnessed(u: &FieldElem, wit: &[u8]) -> Result<PointPrime, ProgramError> {
    ...
    let usq = mont_mul(&u.mont, &u.mont);
    let xi_usq = mont_mul(&SSWU_XI, &usq);
    let tv2 = add_mod(&mont_mul(&xi_usq, &xi_usq), &xi_usq);
    if is_zero(&tv2) {
        return Err(ProgramError::InvalidInstructionData);
...
pub mod witness {
    ...
    fn inverse(v_m: &Fp) -> [u8; 48] {
        limbs_to_be(&from_mont(&pow_mont(v_m, &exp_inverse())))
    }
    ...
    pub fn generate(msg: &[u8]) -> Vec<u8> {
        ...
        for elem in &u {
            let usq = mont_mul(&elem.mont, &elem.mont);
            let xi_usq = mont_mul(&SSWU_XI, &usq);
            let tv2 = add_mod(&mont_mul(&xi_usq, &xi_usq), &xi_usq);
            assert!(!is_zero(&tv2));

            let w_inv = inverse(&tv2);
            let inv_m = to_mont(&be_to_limbs(&w_inv));
            let x1 = mont_mul(&SSWU_C1_NEG_B_OVER_A, &add_mod(&R, &inv_m));
            let gx1 = gx_at(&x1);
        ...
        let inv_m = to_mont(&be_to_limbs(&inverse(&dx)));
