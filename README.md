...
verified with a multiplication or two. The fat G2 layout (837 bytes, for v1
transactions) is the CU floor at ~116k: it ships the SSWU candidates and a
Velu-form isogeny parameter directly and lets the curve syscalls check the
roots. The byte-minimal layout shrinks the witness to 96 bytes by shipping
half of each square root, riding the branch bits on their parity, and
recomputing the one remaining inverse in-program. Host-side witness
generation ships in the same crate.

```rust
use bls381_hash::{dst, hash_to_g1, hash_to_g2_compact_parity, hash_to_g2_fat, hash_to_g2_fat_for_pairing};
...
// the fat G2 layout: an 837-byte blob and the lowest CU. A verifier that
// feeds the point to the pairing syscall can skip the final validate
let point = hash_to_g2_fat(dst::G2_RO, payload)?;
let point = hash_to_g2_fat_for_pairing(dst::G2_RO, payload)?;
...
| `g1-ro`, `g2-ro` | standard `_SSWU_RO_POP_` pipelines, `g2-ro` with the fat, compact, xgcd and parity blob layouts |
...
An end-to-end min-pk BLS verify (hash_to_G2 plus the pairing syscall) at
k=14 lands around 169k CU with the fat blob (154k with the signature and
absentee keys uncompressed), 245k default, 391k compact, 430k parity. For scale,
...
The G2 layouts trade witness bytes for CU, and every one runs the same
checks to byte-identical output, so the choice is per call site. v1
transactions hold up to 4 KiB, so the fat 837 B blob is the default for
anything that can send one: ~116k CU, about 40% under the 530 B layout,
and below the zero-witness `modexp` path (~257k) as well. In a legacy
transaction (1,232 bytes) the 96 B parity blob fits every BLS-verifying
instruction surveyed (committee votes, certifications with a 648 B
storage proof, key registration with a second signer) at ~375k, and the
145 B blob buys ~39k CU back for 49 bytes when there is room. Once
SIMD-0529 (`big_mod_exp`) activates, the `modexp` feature's
`hash_to_g2_modexp` replaces the byte-bound blobs at zero witness bytes.
...
The fat layout goes the other way and ships values the pipeline uses:

- the SSWU candidate x itself, pinned by one product against a constant
  per branch (the x2 form never squares tv1)
- sigma = 4 / (x - x_k) for the iso-3 kernel point x_k = -6 + 6i: in
  Velu form both denominators are powers of that difference, and the
  numerators reduce to integers below 16
- no curve check on E': each map lands on E alone, the add syscall that
  joins them checks both images on the curve, and an image on E proves
  its root because Y^2 - X^3 - B = F^2 (y^2 - g'(x))
- each field element as its Montgomery form and quotient, checked by one
  redc and a 65-product integer compare
- every multiply by a fixed constant runs through precomputed lane tables
  at half a Montgomery multiply
...
## Algorithms and references (27 total)

Curve and hash construction (16):
...
- G2 cofactor clearing: Budroni and Pintore
  ...
  optimization slot.
- The fat layout's isogeny form: Velu, "Isogenies entre courbes
  elliptiques", C. R. Acad. Sci. Paris 273 (1971), where a degree-3
  isogeny is x plus two terms in 1 / (x - x_k), and `tools/velu_iso3.py`
  derives the RFC map in that form.
...
- `tools/velu_iso3.py`, `gen_fixed.py`, `gen_quotient.py`: the fat layout's isogeny constants and pin equations, the fixed-multiplier kernel, and the hash_to_field quotient check
